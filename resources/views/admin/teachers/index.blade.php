<x-admin-layout>
    <div class="bg-white rounded-[1rem] shadow-sm border border-gray-100 p-6">
        {{-- --- Filter --- --}}
        <div class="mb-6">
            <form action="{{ route('admin.teachers.index') }}" method="GET" class="flex flex-wrap gap-3 items-end">

                {{-- search bar --}}
                <div>
                    <label class="block text-xs font-bold text-gray-500 mb-1 uppercase tracking-wider">Search User</label>
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="Name or Email..."
                        class="form-control w-64 border-gray-300 rounded-md shadow-sm text-sm focus:ring-blue-500">
                </div>
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('admin.teachers.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-md text-sm font-bold transition-colors">
                        Reset
                    </a>
                    @endif
                </div>
            ...
            <table class="w-full min-w-[900px] text-left border-collapse">
                <thead>
                    <tr class="border-b border-gray-200">
                        <th class="px-6 py-4 font-bold text-gray-700 text-center">teacher name</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr x-data="{ active: {{ $teacher->status ? 'true' : 'false'}}, open: false, showModal: false}">
                        ...
                        {{-- e-mail--}}
                        <td class="px-6 py-4 text-gray-600 text-center text-sm">{{ $teacher->email }}</td>

                        {{-- created-at --}}
                        <td class="px-6 py-4 text-gray-500 text-center text-sm">{{ $teacher->created_at->format('Y-m-d') }}</td>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="relative inline-block text-left">
                                ...
                                <div x-show="open" @click.away="open = false"
                                    ...
                                    {{--1 view profile --}}
                                    <a href="{{ route('profile.show',$teacher->id)}}" class="group block px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 flex items-center">
                                    ...
                                    <button @click="showModal = true" type="button"
                                        ...
                                        <span x-text="active ? 'Deactivate {{$teacher->name}}' : 'Activate {{$teacher->name}}'"></span>
                                    </button>
                            ...
                            <div x-show="showModal"
                                ...
                                <div @click.away="showModal = false"
                                    ...
                                    <div class="p-6 text-left">
                                        <p class="text-gray-700">Are you sure you want to <span x-text="active ? 'deactivate' : 'activate'" class="font-bold"></span> <span class="font-bold text-black">{{$teacher->name}}</span>?</p>
                                    </div>
